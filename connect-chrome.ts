...
async function handleDiscoveryCard(page: Page): Promise<void> {
  try {
    await page.waitForSelector('button[aria-label="Acknowledge and close the discovery card."]', { timeout: 10000 });
    await page.click('button[aria-label="Acknowledge and close the discovery card."]');
  } catch (e) {
    // ignore if not found
  }
}
...
async function waitForCopyButton(page: Page): Promise<void> {
  try {
    await page.waitForSelector('response-container:last-child copy-button', { timeout: 500 });
